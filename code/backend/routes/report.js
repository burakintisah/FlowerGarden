...
router.get('/seller/:id', (req, res) => {
    ...
    var query = 'SELECT district_id, district_name, min(order_date) as first_order, count(order_id) as number_of_orders, '
        + ' sum(price) as total_revenue, avg(price) as average_revenue, avg(rate) as average_rate FROM flowergarden.order '
        + ' NATURAL JOIN flower_arrangement NATURAL JOIN district WHERE seller_id = ? GROUP BY district_id ORDER BY average_revenue';
